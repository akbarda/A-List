...
import com.maderaiindra.alarmlista_list.DTO.ToDoItem

class DBHandler(val context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    override fun onCreate(db: SQLiteDatabase) {
        val createToDoTable = "  CREATE TABLE $TABLE_TODO (" +      //Membuat Table
                "$COL_ID integer PRIMARY KEY AUTOINCREMENT," +      //Menentukan primary key
                "$COL_CREATED_AT datetime DEFAULT CURRENT_TIMESTAMP," +      //coloumn createdAt
                "$COL_NAME varchar);"                                   //Coloumn Name
        val createToDoItemTable =
            "CREATE TABLE $TABLE_TODO_ITEM (" +                     //Pembuatan table todo_item
                    "$COL_ID integer PRIMARY KEY AUTOINCREMENT," +      //Menentukan primary key
                    "$COL_CREATED_AT datetime DEFAULT CURRENT_TIMESTAMP," +     //Deklarasi tipe data Coloumn createdAt
                    "$COL_TODO_ID integer," +                                   //Deklarasi tipe data coloumn id
                    "$COL_ITEM_NAME varchar," +                                 //Deklarasi tipe data coloumn itemname
                    "$COL_IS_COLPLETED integer);"                           //Deklarasi tipe data coloumn colpleted
